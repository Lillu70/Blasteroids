...
struct General_Allocator
{
    ...
    void init(u32 _capacity, bool _crash_on_out_of_memory=true)
    {
        if(owns_memory)
        {
            Assert(memory);
        ...
        void* _memory = Platform_Allocate_Memory(_capacity, &_capacity);
...
{    
    ...
    void resize_page_table(u32 new_table_capacity)
    {
        ...
        General_Allocator* new_table = 
            (General_Allocator*)Platform_Allocate_Memory(alloc_size, &result_alloc_size);
        ...
        if(page_table)
        {
            ...
            Platform_Free_Memory(page_table);
        }
    ...
    void init(u32 _page_count=255, u32 _default_page_size = MiB)
    {
        ...
        active_page_count = 1;
        active_page_idx = 0;
        (*page_table).init(default_page_size, false);
    }
    ...
    void* push(u32 size)
    {
        ...
        if(!result)
        {
            ...
            if(!result)
            {
                ...
                page_table[active_page_idx].init(page_size, false);
...
static inline void Init_Shell_From_Linear_Allocator(
    Allocator_Shell* shell, 
    Linear_Allocator* linear_allocator)
{
    shell->internal_alloc_adr = linear_allocator;
    shell->_push = Linear_Allocator::_Static_Push;
    shell->_free = 0;
    shell->min_alloc_size = 1;
}
...
#if 0
// Don't think I want to include stdlib in ANY project, 
