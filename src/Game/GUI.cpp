...
static void gui_push_frame(GUI_Handler* handler, 
...
static void gui_pop_frame(GUI_Handler* handler, Platform_Call_Table* platform, General_Allocator* mem_arena)
{
    ...
    {    
        ...
        *active_frame = *prev_frame;
        mem_arena->free(prev_frame); // This will free the GUI_Frame*
    }
...
static void gui_create_header(GUI_Handler* handler, 
...
static GUI_Widget_Header* gui_add_button(GUI_Handler* handler, 
...
static GUI_Widget_Header* gui_add_text(GUI_Handler* handler, 
    GUI_Text_Spec* spec, 
    v2f* position,
    GUI_Link_Direction ld)
{
...
static GUI_Widget_Header* gui_add_slider(GUI_Handler* handler, 
    ...
    GUI_Link_Direction ld)
{
 ...
 static GUI_Widget_Header* gui_add_checkbox(GUI_Handler* handler, 
		...
		switch(header->type)
		{
			case GUI_Widget_Type::button:
			{
				...
				if(spec->text)
				{
					...
					canvas->draw_text((char*)spec->text, 
									  text_p, 
									  outline_color, 
			...
			case GUI_Widget_Type::slider:
			{
                ...
                canvas->draw_percentile_bar(bar_start, 
                                            bar_end, 
                                            fill, 
                                            fill_color, 
                                            theme->background_color, 
            ...
            case GUI_Widget_Type::checkbox:
            {
				...
				canvas->draw_filled_rect_with_outline(rect, 
													  theme->outline_thickness,
													  outline_color,
													  theme->background_color);
...
static inline GUI_Additional_Highlights* gui_setup_additional_highlighting(GUI_Handler* handler, 
...
static inline void gui_add_additional_highlight(GUI_Additional_Highlights* addhigh, 
    GUI_Widget_Header* widget)
{
