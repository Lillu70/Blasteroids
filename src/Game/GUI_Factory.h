...
// Due to dumass design by me (Valtteri), these functions have be decalered here.
...
{
	Platform_Set_Fullscreen(spec->is_checked);
...
static void gui_create_rebind_menu(Action* action_array, u32 action_idx)
{
	...
	gui_push_frame(&game.gui_handler, &mem, menu_memory_size, NO_BACK_ACTION);
	
	// Header text.
	{
		GUI_Text_Spec text = {};
		
		text.text = (u8*)"Press any key or controller button to rebind action.";
		text.text_scale = 1;
		text.is_title = true;
	...
	listener.action_array = action_array;
	listener.action_idx = action_idx;
	listener.on_trigger = gui_rebind_action;
...
static void gui_create_main_menu()
{
	...
	// Start game button.
	{
		...
		button.text = (u8*)"Start Game";
		button.text_scale = text_scale;
		button.on_click = set_mode_asteroids_sp;
	...
	{
		GUI_Button_Spec button = {};
		button.text = (u8*)"Leaderboard";
		button.text_scale = text_scale;
		button.on_click = gui_create_leaderboard_menu;
	...
	{
		GUI_Button_Spec button = {};
		button.text = (u8*)"Settings";
		button.text_scale = text_scale;
		button.on_click = gui_create_settings_menu_from_main_menu;
	...
	{
		GUI_Button_Spec button = {};
		button.text = (u8*)"Quit Game";
		button.text_scale = text_scale;
		button.on_click = quit_to_desktop;
...
static void gui_create_pause_menu()
{
	...
	gui_push_frame(&game.gui_handler, &mem, menu_memory_size, unpause_game);
	...
	// Continue button.
	{
		...
		button.text = (u8*)"Continue";
		button.text_scale = text_scale;
		button.on_click = unpause_game;
	...
	// Settings button.
	{
		...
		button.text = (u8*)"Settings";
		button.text_scale = text_scale;
		button.on_click = gui_create_settings_menu_from_pause_menu;
	...
	// Main menu.
	{
		...
		button.text = (u8*)"Main Menu";
		button.text_scale = text_scale;
		button.on_click = set_mode_main_menu;
...
static void gui_create_quit_menu()
{
	...
	gui_push_frame(&game.gui_handler, &mem, menu_memory_size, NO_BACK_ACTION, on_frame_close);  
...
static void gui_create_settings_menu(u8* back_button_name, void(*create_function)(), void(*on_esc_fuck)(), f32 vertical_offset)
{
	...
	gui_push_frame(&game.gui_handler,  &mem, menu_memory_size, gui_save_and_pop_menu);
	...
	// Main Menu button.
	{
		...
		button.text = back_button_name;
		button.text_scale = text_scale;
		button.on_click = gui_save_and_pop_menu;
	...
	// Keybinds button.
	{
		...
		button.text = (u8*)"Controls";
		button.text_scale = text_scale;
		button.on_click = gui_create_controls_menu;
	...
	GUI_Additional_Highlights* fullscreen_checkbox_highlights;
	{
		...
		checkbox.is_checked = (Platform_Get_Flags() & Platform_Flags::fullscreen) > 0;
		checkbox.on_value_change = gui_set_fullscreen;
	...
	Action* action)
{
	...
	button.text_scale = common_data->text_scale;
	button.on_click = create_rebind_menu;
	...
	
	{
		...
		text.text = key_name;
		text.text_scale = common_data->text_scale;
...
static void gui_create_controls_menu()
{
	...
	gui_push_frame(&game.gui_handler, &mem, menu_memory_size, gui_prev_menu);
	...
	// Game controls button.
	{
		...
		button.text = (u8*)"Game";
		button.text_scale = text_scale;
		button.on_click = gui_create_game_controls_menu;
	...
	// Menu controls button.
	{
		...
		button.text = (u8*)"Menu";
		button.text_scale = text_scale;
		button.on_click = gui_create_menu_controls_menu;
	...
	// Global controls button.
	{
		...
		button.text = (u8*)"Global";
		button.text_scale = text_scale;
		button.on_click = gui_create_global_controls_menu;
...
static void gui_create_game_controls_menu()
{
	...
	common_data.text_scale = text_scale;
	common_data.keyboard_text_offset = keyboard_text_offset;
	common_data.control_text_offset = control_text_offset;
	common_data.dimensions = dimensions;
...
static void gui_create_leaderboard_menu()
{
	...
	if(s_highscores[0] == 0)
	{
		...
		prefix_text.text = (u8*)"You haven't scored anythin yet. Go play the game!";
		prefix_text.text_scale = 1;
		prefix_text.is_title = true;
	...
	else
	{
		for(u32 i = 0; i < s_max_highscore_count; ++i)
		{
			...
			s32 used_scale = scale < 1? 1 : scale;
			scale -= 1;
			...
			prefix_text.text = (u8*)listin_prefixes[i];
			prefix_text.text_scale = used_scale;
			prefix_text.is_title = true;
